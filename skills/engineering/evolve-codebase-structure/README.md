...
- **选择最小充分结构。** 每个候选都要说明为什么应保持、拆成文件、形成子目录、升级为 package、合并、迁移或重命名，以及为什么相邻的更小或更大层级不合适。
- **小公共接口，清晰私有实现。** 允许拆分实现文件，但不会为了缩短文件而增加公共接口或制造跳转。
- **结构收益必须可验收。** 目录更漂亮不算收益；候选必须说明它怎样改善 locality、接口、依赖、导航、ownership 或 testability，以及下游实现怎样验证。
- **允许“无需重构”。** 没有足够证据时，正确输出可以是当前结构合理，而不是为了填满报告硬造候选。

## 如何判断结构调整成功

具体判断规则以 [`STRUCTURE-DESIGN.md`](STRUCTURE-DESIGN.md) 为唯一规范来源。报告和访谈不会复制一套简化规则，而会引用其中的 placement ladder 与 Structural Success Contract。

每个候选必须选择最小充分的结构动作，并解释为什么更小的动作解决不了问题、为什么更大的动作属于过度设计。最终验收关注的是结构结果：变化是否更局部、公共接口是否更有意、依赖是否更清楚、所有权是否更准确、导航是否更有意义，或测试是否更自然。

现有测试、构建、import 和兼容性检查只用于证明搬迁没有破坏原有行为，不能反过来证明结构已经改善。性能验证也不是默认步骤；只有当结构变化触及 import/loading、动态分发、registry/plugin、进程或 IPC、GPU 热路径、同步或 package/build loading 等运行时敏感机制时才需要。
...
1. Explore：检查树、依赖、历史、测试、构建和所有权
   ↓
2. HTML report：展示证据、结构层级、成功假设、风险和首选项
   ↓  你选择一个候选
...
- 不把测试通过或性能不变误写成结构改善本身。
- 不默认要求性能基准；只有运行时敏感机制发生变化时才验证相关不变量。
- 不在审查过程中直接重构源码。
