...
Run a periodic, read-first architecture survey focused on physical code organization: where behavior belongs, when a file should gain private structure, when files should merge, and when a real package boundary has emerged.
...
Treat line count as a prompt to inspect cohesion. Apply the candidate tests in `STRUCTURE-DESIGN.md`, record counterevidence, and discard decorative splits. For an upcoming change, admit `Prospective` candidates only when the user's stated responsibility, dependency, runtime, or lifecycle creates a concrete pressure against the repository's present seams. Preserve uncertain ideas as hypotheses with `Speculative` strength.

The exploration is complete when the bounded scope and evidence sources are explicit, each inspected hotspot is either a candidate or a rejected false positive with a reason, and every candidate passes the quality gate in `STRUCTURE-DESIGN.md`. Zero actionable candidates is a valid result.
...
Rank candidates by expected payoff, current relevance, confidence, and migration risk. Put at most five principal candidates in the main report; keep weaker hypotheses and rejected false positives in the appendix. Every card must separate observation from inference and show a compact current versus possible tree or dependency view. Describe a provisional interface hypothesis only far enough to make the structural move intelligible; reserve the final interface and unresolved ownership choices for discussion.

The report phase is complete when every candidate is traceable to concrete evidence, counterevidence is visible, the top recommendation is justified, and the report can honestly say that no structural change is warranted.
...
- one caller-facing interface, private implementation roles, and permitted dependency direction;
- split, merge, relocation, or boundary alternatives, including the do-nothing option;
...
- compatibility, rollback, performance, runtime, deployment, and hardware constraints;
...
The grilling loop is complete when no unresolved decision blocks a spec, every remaining empirical unknown has a concrete validation plan or downstream ticket, and the user confirms shared understanding. Record a **Structure Decision Brief**:
...
Build/runtime/compatibility constraints:
...
Present the completed brief and recommend the next explicit user-invoked step. A user-invoked skill cannot invoke another user-invoked skill. In Codex, invite the user to run:
